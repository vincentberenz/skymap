...
PATCH_SIZE = 200  # Initial size of the central patch to extract in pixels
...
def calculate_fov_estimate(image_shape: Tuple[int, int], patch_size: int) -> Tuple[u.Quantity, int, float]:
    """
    ...
    if search_radius > MAX_GAIA_RADIUS:
        ...
        if adjusted_multiplier > 1.0:
            adjusted_multiplier = min(adjusted_multiplier, 
                                   (MAX_GAIA_RADIUS * 2) / fov_estimate)
            search_radius = fov_estimate * adjusted_multiplier / 2.0
        
        # If still too large, reduce the patch size
        if search_radius > MAX_GAIA_RADIUS:
            # Calculate the maximum patch diagonal that would keep us within limits
            max_patch_diag = (2 * MAX_GAIA_RADIUS * full_diag) / (FISHEYE_FOV * adjusted_multiplier * 1.05)
            adjusted_patch_size = int((max_patch_diag / np.sqrt(2)) * 0.9)  # 90% to be safe
    ...
    logger.debug(f"Adjusted parameters - Patch size: {adjusted_patch_size}, "
...
def solve_plate(
    ...
    try:
        ...
        stars_xy = find_peaks(image_data, threshold=STAR_DETECTION_THRESHOLD)[0:MAX_STARS]
        ...
        logger.debug(f"Querying GAIA catalog around {center_coords} with FOV {search_fov:.2f}")
        ...
        try:
            ...
            if len(gaia_stars) == 0:
                raise ValueError("No stars found in GAIA catalog for the given coordinates and FOV")
                
            if len(gaia_stars) == MAX_GAIA_STARS:
                logger.warning(f"Reached maximum number of stars ({MAX_GAIA_STARS}) in GAIA query. "
...
def main() -> None:
    """Main function to run the plate solving process."""
    ...
    try:
        ...
        logger.info(f"Using FOV multiplier: {fov_multiplier:.2f} (original: {FOV_MULTIPLIER})")
        
        # Now process the image with the adjusted patch size
        image_data: np.ndarray = load_and_process_fisheye_image(IMAGE_PATH, patch_size=adjusted_patch_size)
        ...
        if wcs is not None:
            ...
            # Save WCS solution
            # output_path = Path(IMAGE_PATH).with_suffix('.wcs')
            # wcs.write(output_path)
            # logger.info(f"WCS solution saved to {output_path}")
        else:
